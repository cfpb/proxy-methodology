...
foreach file in `geo_files'{
   use "`indir'`file'_over18_race_jan20.dta", clear
   	gen file = "`file'"
    
* Step 1: From the DHC, retain population counts for the contiguous U.S., Alaska, and Hawaii in order to ensure consistency with the population
* covered by the census surname list.
    drop if State_FIPS20 == "72"

    if "`file'" == "zip" {
	drop if inlist(substr(ZCTA5,1,3),"006","007","008","009")
	}

* Step 2: Address "Other" category from 2020 Census; what is done here follows Word(2008).
...
* for each group.
    ...
    if file == "blkgrp"{
    	rename GEOID20_BlkGrp GeoInd
    }
    if file == "tract"{
    	rename GEOID20_Tract GeoInd
    }
    if file=="zip"{
    	rename ZCTA5 GeoInd
    }     
    ...
    compress
    save "`outdir'`file'_attr_over18_jan20.dta", replace
}
